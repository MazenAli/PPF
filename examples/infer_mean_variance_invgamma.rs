...
fn main() {
    ...
    println!(
        "Data mean: {:.3}, Data std: {:.3}, Data variance: {:.3}",
        data_mean, data_std, data_var
    );

    // Define a model: Normal likelihood with unknown mean and variance
    // Parameters: [mu, log_sigma] where we use Inverse-Gamma prior on σ²
    let model = Model::new(move |params: &[f64]| {
        ...
        let prior_variance = InverseGamma::new(2.0, 1.0).log_prob(variance);
        ...
        prior_mu + prior_variance + likelihood
    });

    // Inference: run Metropolis-Hastings
    println!("\nRunning MCMC inference...");
    let mh = MetropolisHastings::new(2, 0.08); // 2 parameters, small proposal std for stability
    let samples = mh.sample(&model, 15_000);

    // Analysis (skip burn-in)
    let burn_in = samples.len() / 3;
    println!(
    ...
    // Transform log_sigma samples back to sigma and variance scales
    let sigma_samples: Vec<f64> = samples.get_param(1).iter().map(|&x| x.exp()).collect();
    let variance_samples: Vec<f64> = sigma_samples.iter().map(|&x| x * x).collect();

    let posterior_sigma = sigma_samples.iter().sum::<f64>() / sigma_samples.len() as f64;
    let posterior_variance = variance_samples.iter().sum::<f64>() / variance_samples.len() as f64;
    ...
    // Compute sigma and variance credible intervals
    ...
    let mut sorted_variance = variance_samples.clone();
    sorted_variance.sort_by(|a, b| a.partial_cmp(b).unwrap());
    let var_q025 = sorted_variance[(0.025 * sorted_variance.len() as f64) as usize];
    let var_q975 = sorted_variance[(0.975 * sorted_variance.len() as f64) as usize];

    println!("Std: [{:.3}, {:.3}]", sigma_q025, sigma_q975);
    println!("Variance: [{:.3}, {:.3}]", var_q025, var_q975);
    ...
    println!(
        "Data variance: {:.3} | Posterior variance: {:.3} | Difference: {:.3}",
        data_var,
        posterior_variance,
        (data_var - posterior_variance).abs()
    );

    // Prior specification and conjugacy information
    println!("\n=== PRIOR SPECIFICATION ===");
    ...
    println!(
        "New observation ~ Normal({:.3}, {:.3})",
        posterior_mu, posterior_sigma
    );
    println!(
        "95% prediction interval: [{:.3}, {:.3}]",
        posterior_mu - 1.96 * posterior_sigma,
        posterior_mu + 1.96 * posterior_sigma
    );
}
